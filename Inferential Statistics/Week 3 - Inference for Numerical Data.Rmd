...
```{r habit-weight-box}
boxplot(weight ~ habit, data = nc, ylab = "weightlb")
...
```{r gained-mature-ht}
inference(x = mature, y = gained, data = nc, statistic = "mean", type = "ht", null = 0, alternative = "twosided", method = "theoretical" )
...
```{r pick-your-own}
#Determine if race of mother impacts weight of babies
#Explanatory = race, response = weight
#p value = 0.05 since CI = 95%

inference(data = nc, y = weight, x = whitemom, statistic = "mean", null = 0, alternative = "twosided", type = "ht", method = "theoretical")

inference(data = nc, y = weight, x = whitemom, statistic = "mean", type = "ci", method = "theoretical", order = "white", "not white")

#conclude that we can reject null hypothesis and that we are 95% confident that babies born to white mothers are on average 0.3142 to 0.7477 lbs heavier.
```
