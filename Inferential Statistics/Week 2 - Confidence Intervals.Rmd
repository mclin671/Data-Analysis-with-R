...
```{r set-seed}
set.seed(06222022)                 # make sure to change the seed
```
...
```{r describe-sample}
ggplot(data = samp, aes(x = area)) + geom_histogram(binwidth = 250)
...
```{r plot-99-perc-cis}
#generate 50 samples and their lower/upper bounds for a 99% confidence interval
ci <- ames %>%
 rep_sample_n(size = n, reps = 50, replace = TRUE) %>%
 summarise(lower = mean(area) - z_star_99 * (sd(area)/sqrt(n)), upper = mean(area) + z_star_99 * (sd(area)/sqrt(n)))

#generate new column to determine if sample mean falls within population mean calculated
ci <- ci %>%
 mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))

#sort data to have lower and upper bound separate
ci_data <- gather(ci, type, bound, lower:upper)

#plot data
ggplot(ci_data, aes(x = bound, y = replicate, group = replicate, color = capture_mu)) + geom_point(size = 2) + geom_line() + geom_vline(xintercept = params$mu, color = "darkgray")
