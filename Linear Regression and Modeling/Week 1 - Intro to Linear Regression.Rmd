...
```{r residual}
y_obs <- mlb11 %>% filter(at_bats == 5579) %>% select(runs)

y_exp <- -2789.2429 + 0.6305*5579
...
```{r runs-vs-sth-else}
#hit analysis
ggplot(data = mlb11, aes(x = hits, y = runs), inherit.aes = FALSE) + geom_point() + stat_smooth(method = "lm", se = FALSE)
m3 <- lm(runs ~ hits, data = mlb11) 
summary(m3)


#bat_avg analysis
ggplot(data = mlb11, aes(x = bat_avg, y = runs), inherit.aes = FALSE) + geom_point() + stat_smooth(method = "lm", se = FALSE)
m4 <- lm(runs ~ bat_avg, data = mlb11)
summary(m4)

#strikeout analysis
ggplot(data = mlb11, aes(x = strikeouts, y = runs), inherit.aes = FALSE) + geom_point() + stat_smooth(method = "lm", se = FALSE)
m5 <- lm(runs ~ strikeouts, data = mlb11) 
summary(m5)

#stolen_bases
ggplot(data = mlb11, aes(x = stolen_bases, y = runs), inherit.aes = FALSE) + geom_point() + stat_smooth(method = "lm", se = FALSE)
m6 <- lm(runs ~ stolen_bases, data = mlb11)
summary(m6)

#wins
ggplot(data = mlb11, aes(x = wins, y = runs), inherit.aes = FALSE) + geom_point() + stat_smooth(method = "lm", se = FALSE)
...
```{r diag}
ggplot(data = n1, aes(x = .resid)) + geom_histogram(binwidth = 25) + xlab("Residuals")

ggplot(data = n1, aes(sample = .resid)) + stat_qq()
```
