...
ggplot(data = samp1, aes(x = area)) + geom_histogram(binwidth = 250)
 samp1 %>%
     summarise(mean_50 = mean(area), med_50 = median(area), 
               sigma_50 = sd(area), iqr_50 = IQR(area),
               min_50 = min(area), max_50 = max(area),
               q1_50 = quantile(area, 0.25),  # first quartile, 25th percentile
               q3_50 = quantile(area, 0.75))  # third quartile, 75th percentile
...
```{r sampling-dist}
 sample_means50 %>%
    summarise(mean_50 = mean(x_bar), med_50 = median(x_bar), 
            sigma_50 = sd(x_bar), iqr_50 = IQR(x_bar),
            min_50 = min(x_bar), max_50 = max(x_bar),
            q1_50 = quantile(x_bar, 0.25),  # first quartile, 25th percentile
            q3_50 = quantile(x_bar, 0.75))  # third quartile, 75th percentile
...
sample_means150 <- ames %>%
 ...
 summarise(x_bar = mean(price))
ggplot(data = sample_means150, aes(x = x_bar)) + geom_histogram(binwidth = 250)
